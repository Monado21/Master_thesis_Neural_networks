...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, filepaths, N_sub=10, batch_size=16, batch_multiplier=10):
        self.filepaths = filepaths
        self.N_samples = len(filepaths)
        self.N_sub = N_sub
    ...
    def __len__(self):
        return self.batch_multiplier * self.N_samples * (self.Nx // (self.batch_size * self.N_sub))
    ...
    def on_epoch_end(self):
        ...
        self.Nt = self.X.shape[2]
        self.station_inds = np.arange(self.Nx - self.N_sub)

    def __getitem__(self, idx):
        batch_size = self.batch_size
        selection = slice(idx * batch_size, (idx + 1) * batch_size)
        samples, masked_samples, masks = self.__data_generation(selection)
        samples = np.expand_dims(samples, -1)
        masked_samples = np.expand_dims(masked_samples, -1)
        masks = np.expand_dims(masks, -1)
        return (samples, masks), masked_samples

    def __data_generation(self, selection):
        samples = np.zeros((self.batch_size, self.N_sub, self.Nt))
        masks = np.ones_like(samples)
        masked_samples = np.zeros_like(samples)
        for i, j in enumerate(range(selection.start, selection.stop)):
            s = j // (self.Nx - self.N_sub)
            station = j % (self.Nx - self.N_sub)
            station_slice = slice(station, station + self.N_sub)
            sample = self.X[s, station_slice, :]
            # Random time reversal
            order = rng.integers(low=0, high=2) * 2 - 1
            sample = sample[:, ::order]
            sign = rng.integers(low=0, high=2) * 2 - 1
            subsample = sign * sample
            samples[i] = subsample
            # Select three waveforms to blank
            blank_inds = rng.choice(self.N_sub, size=3, replace=False)
            masks[i, blank_inds] = 0
            masked_samples[i] = subsample * (1 - masks[i])
...
class UNet:
    ...
    def construct(self):
        ...
        """ Encoder """
        x = self.conv_layer(x, filters=f*2, kernel_size=kernel,
                            use_bn=use_bn, use_dropout=use_dropout,
                            activ=activation)
        x = self.conv_layer(x, filters=f*4, kernel_size=kernel,
                            use_bn=use_bn, use_dropout=use_dropout,
                            activ=activation)
        x = self.conv_layer(x, filters=f*8, kernel_size=kernel,
                    use_bn=use_bn, use_dropout=use_dropout,
                    activ=activation)
        x = self.conv_layer(x, filters=f*12, kernel_size=kernel,
                    use_bn=use_bn, use_dropout=use_dropout,
                    activ=activation)
